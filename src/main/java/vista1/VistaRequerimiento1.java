...
public class VistaRequerimiento1 extends JFrame {
    ...
    String[] titulo = { "Ciudad", "Acabados", "Clasificación", "Banco", "Constructora" };

    public VistaRequerimiento1() {
        super("Requerimiento 1");
        initUITable();
    ...
    private String[][] obtenerResultados() throws SQLException {
        ...
        for (int i = 0; i < ArrListRS.size(); i++) {
            matrizDeDatos[i][0] = ArrListRS.get(i).getCiudad();
            matrizDeDatos[i][1] = ArrListRS.get(i).getAcabados();
            matrizDeDatos[i][2] = ArrListRS.get(i).getClasificacion();
            matrizDeDatos[i][3] = ArrListRS.get(i).getBanco_Vinculado();
            matrizDeDatos[i][4] = ArrListRS.get(i).getConstructora();
        }
